...
class InputSearchPhoneNumber extends Component
{
    ...
    {
        if (strlen($this->phone_number) > 2) {
            $data = file_get_contents('http://localhost:777/billingCorporation/public/api/newTelecomeBill?searsh=' . $this->phone_number);
            $data = json_decode($data);
            $bills = $data->data;
            if ($bills == "هذا الرقم غير موجود") {
                $dataar = file_get_contents('http://localhost:777/billingCorporation/public/api/archivedTelecomeBill?searsh=' . $this->phone_number);
