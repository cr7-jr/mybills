...
class billTelecomeController extends Controller
{
    ...
    public function pay($phone_number, $course_number, $id_bank, $send_email = true)
    {
        ...
        $bill = file_get_contents(Billing_CORPORATION_DOMAIN_NAME . "api/getNewTelecomeBill/$phone_number/$course_number");
        $bill = json_decode($bill);
        $bill = $bill->data;
        //dd($bill);
        $amount_due_of_payment = $bill->amount_due_of_payment;
        $result_check = file_get_contents(BANK_DOMAIN_NAME . 'api/checkInformation/' . $id_bank . '/value/' . $amount_due_of_payment . '?number=' . $phone_number . '&course_number=' . $course_number . '&' . 'bill_type=telecome');

        $result_check = json_decode($result_check);

        if ($result_check[0] == true) {
            file_get_contents(Billing_CORPORATION_DOMAIN_NAME . 'api/complatePayTelecome/' . $phone_number . '/' . $course_number . '/' . $result_check[1] . '/' . auth()->id());
    ...
    {
        $bill = file_get_contents(Billing_CORPORATION_DOMAIN_NAME . 'api/getArchivedTelecomeBill/' . $phone_number . '/' . $course_number);
        $bill = json_decode($bill);
        $bill = $bill->data;
    ...
    public  function archived(Request $request)
    {
        ...
        $telecome_statistic = file_get_contents(Billing_CORPORATION_DOMAIN_NAME . 'api/telecome/RestoreConsumptionRates?phone_number=' . $request->phone_number);
        $telecome_statistic = json_decode($telecome_statistic);
        $statistic = $telecome_statistic->data;
        $code = $telecome_statistic->code;
        $phone_number = $request->phone_number;
        $data = file_get_contents(Billing_CORPORATION_DOMAIN_NAME . 'api/archivedTelecomeBill?searsh=' . $phone_number);
