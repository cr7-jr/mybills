...
class billWaterController extends Controller
{
    ...
    {
        $bill = file_get_contents(Billing_CORPORATION_DOMAIN_NAME . "api/getNewWaterBill/$counter_number/$course_number");
        $bill = json_decode($bill);
        $bill = $bill->data;
        $amount_due_of_payment = $bill->amount_due_of_payment;
        $result_check = file_get_contents(BANK_DOMAIN_NAME . 'api/checkInformation/' . $id_bank . '/value/' . $amount_due_of_payment . '?number=' . $counter_number . '&course_number=' . $course_number . '&' . 'bill_type=water');
        $result_check = json_decode($result_check);
        if ($result_check[0] == true) {
            file_get_contents(Billing_CORPORATION_DOMAIN_NAME . 'api/complatePayWater/' . $counter_number . '/' . $course_number . '/' . $result_check[1] . '/' . auth()->id());
            ...
            session()->flash('msg', 'االرصيد غير كافي لدفع هذه الفاتورة');
            return \redirect('http://127.0.0.1:8000/en/myBills/new/water?counter_number=' . $counter_number);
        }
    } //end pay
    ...
    {
        $bill = file_get_contents(Billing_CORPORATION_DOMAIN_NAME . 'api/getArchivedWaterBill/' . $counter_number . '/' . $course_number);
        $bill = json_decode($bill);
        $bill = $bill->data;
    ...
    public  function archived(Request $request)
    {
        ...
        $water_statistic = file_get_contents(Billing_CORPORATION_DOMAIN_NAME . 'api/water/RestoreConsumptionRates?counter_number=' . $request->counter_number);
        ...
        $data = file_get_contents(Billing_CORPORATION_DOMAIN_NAME . 'api/archivedWaterBill?searsh=' . $counter_number);
